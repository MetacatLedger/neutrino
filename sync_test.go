...
var (
	// Try czzlog.LevelInfo for output like you'd see in normal operation,
	// or czzlog.LevelTrace to help debug code. Anything but
	// czzlog.LevelOff turns on log messages from the tests themselves as
	// well. Keep in mind some log messages may not appear in order due to
	// use of multiple query goroutines in the tests.
	logLevel    = czzlog.LevelOff
...
func (s *secSource) GetScript(addr czzutil.Address) ([]byte, error) {
...
var (
	...
	secSrc                    *secSource
	addr1, addr2, addr3       czzutil.Address
...
func testStartRescan(harness *neutrinoHarness, t *testing.T) {
	...
	inSrc := func(tx wire.MsgTx) func(target czzutil.Amount) (
		total czzutil.Amount, inputs []*wire.TxIn,
		...
		return func(target czzutil.Amount) (total czzutil.Amount,
			inputs []*wire.TxIn, inputValues []czzutil.Amount,
			...
			inputValues = []czzutil.Amount{
				czzutil.Amount(tx.TxOut[ourIndex].Value)}
	...
	_, err = harness.h1.GenerateAndSubmitBlockWithCustomCoinbaseOutputs(
		[]*czzutil.Tx{}, rpctest.BlockVersion, time.Time{},
		[]wire.TxOut{{
			Value:    0,
			PkScript: []byte{},
		}})
...
func TestNeutrinoSync(t *testing.T) {
	// Set up logging.
	logger := czzlog.NewBackend(os.Stdout)
...
func startRescan(t *testing.T, svc *neutrino.ChainService, addr czzutil.Address,
	...
	rescan := neutrino.NewRescan(
		...
		neutrino.NotificationHandlers(
			rpcclient.NotificationHandlers{
				...
				OnRedeemingTx: func(tx *czzutil.Tx,
					details *btcjson.BlockDetails) {
					rescanMtx.Lock()
					hash, err := chainhash.
						NewHashFromStr(
