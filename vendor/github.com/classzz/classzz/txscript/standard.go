...
// specified address.
func PayToAddrScript(addr czzutil.Address) ([]byte, error) {
	...
	case *czzutil.AddressPubKeyHash:
		...
		return payToPubKeyHashScript(addr.ScriptAddress())
	case *czzutil.LegacyAddressScriptHash:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
				nilAddrErrStr)
		}
...
func MultiSigScript(pubkeys []*czzutil.AddressPubKey, nrequired int) ([]byte, error) {
...
func ExtractPkScriptAddrs(pkScript []byte, chainParams *chaincfg.Params) (ScriptClass, []czzutil.Address, int, error) {
	...
	case PubKeyHashTy:
		...
		requiredSigs = 1
		addr, err := czzutil.NewAddressPubKeyHash(pops[2].data,
			chainParams)
		if err == nil {
	...
	case PubKeyTy:
		...
		requiredSigs = 1
		addr, err := czzutil.NewAddressPubKey(pops[0].data, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case ScriptHashTy:
		...
		requiredSigs = 1
		addr, err := czzutil.NewAddressScriptHashFromHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case MultiSigTy:
		...
		addrs = make([]czzutil.Address, 0, numPubKeys)
		for i := 0; i < numPubKeys; i++ {
			addr, err := czzutil.NewAddressPubKey(pops[i+1].data,
				chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
